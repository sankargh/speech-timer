...
export default function SpeechTimer() {
  ...
  const [stopped,       setStopped]       = useState(false); // true after stop, until reset
  const [elapsed,       setElapsed]       = useState(0);
  const [activeSpeaker, setActiveSpeaker] = useState(null);
  const [records,       setRecords]       = useState([]);
  const [confirmClear,  setConfirmClear]  = useState(false);
  const [exportStatus,  setExportStatus]  = useState(null); // {type:'csv'|'txt', ok:bool}
  ...
  /* ── Text file export — works on all devices including mobile ── */
  function exportTXT() {
    try {
      const total = records.reduce((a, r) => a + r.duration, 0);
      ...
      const header = [
        ...
        "",
        line,
        padR("#",4) + padR("Speaker",18) + padR("Speech Type",24) + padR("Duration",10) + padR("Result",14) + "Date & Time",
        line,
      ];
      ...
      const rows = records.map((r, i) =>
        ...
        padR((r.type.length > 22 ? r.type.slice(0, 21) + "…" : r.type), 24) +
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn { cursor: pointer; border: none; font-family: inherit;
               transition: filter 0.13s, transform 0.1s, box-shadow 0.13s; }
        ...
        .progress-track {
          height: 8px; border-radius: 99px; background: #e2e8f0;
        ...
        /* records */
        .rec-row {
          display: grid; grid-template-columns: 36px 1fr auto auto 36px;
      ...
      {exportStatus && (
        <div className="toast" style={{
          ...
          {exportStatus.ok
            ...
            : `✗ ${exportStatus.type.toUpperCase()} export failed — please try again`}
        </div>
      )}
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "20px 16px 60px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            ...
            <button className="btn" onClick={addSpeaker} style={{
              ...
              background: "#1d4ed8", color: "#fff", boxShadow: "0 1px 4px rgba(29,78,216,0.2)",
            }}>+ Add</button>
          </div>
          ...
          {/* Custom timing inputs — shown only when Custom Speech selected */}
          {isCustom && (
            <div style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setCustomError("");
                      setElapsed(0); setStopped(false); setRunning(false);
                    }}
                    style={{
                      width: "100%", border: "1.5px solid #e2e8f0", borderRadius: 8,
                      padding: "10px 12px", fontSize: 15, color: "#1e293b",
                      background: "#fff", textAlign: "center", fontWeight: 600,
                    }}
                  />
                </div>

                <div style={{ paddingTop: 26, color: "#94a3b8", fontWeight: 700, fontSize: 18 }}>→</div>
        ...
        <div className="controls-grid" style={{ display: "grid", gridTemplateColumns: "1fr", gap: 10 }}>
          ...
          <button
            ...
            onClick={startTimer}
            disabled={!canStart}
            title={isCustom && !customValid ? "Enter a valid Min and Max time above" : ""}
            style={{
              padding: "15px", borderRadius: 12, fontSize: 15, fontWeight: 700,
              background: canStart ? "#16a34a" : "#e2e8f0",
          ...
          <button
            ...
            style={{
              padding: "15px", borderRadius: 12, fontSize: 15, fontWeight: 700,
              background: canStop ? "#dc2626" : "#e2e8f0",
              color:      canStop ? "#fff"     : "#94a3b8",
              letterSpacing: "0.02em",
            }}
          >
            ■  Stop &amp; Record
          ...
          <button
            ...
            style={{
              padding: "15px", borderRadius: 12, fontSize: 15, fontWeight: 700,
              background: canReset ? "#f1f5f9" : "#e2e8f0",
              color:      canReset ? "#475569"  : "#94a3b8",
              border: canReset ? "1.5px solid #cbd5e1" : "1.5px solid #e2e8f0",
              letterSpacing: "0.02em",
            }}
          >
            ↺  Reset Timer
        ...
        {records.length > 0 && (
          <div className="card" style={{ overflow: "hidden" }}>
            ...
            <div style={{
              ...
              {/* Export TXT */}
              <button className="btn" onClick={exportTXT} style={{
                padding: "6px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600,
            ...
            <div style={{
              ...
              gap: "0 12px", padding: "7px 16px", borderBottom: "1px solid #f1f5f9",
            ...
            {records.map(r => (
              <div key={r.id} className={`rec-row${selectedRows.has(r.id) ? " sel" : ""}`}>
                ...
                <button className="btn" onClick={() => deleteRecord(r.id)} title="Delete" style={{
